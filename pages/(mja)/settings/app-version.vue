...
const emptyPlatform = (): PlatformVersion => ({
  latestVersion: '',
  minRequiredVersion: '',
  storeUrl: '',
  releaseNotes: '',
...
<template>
  <div class="mx-auto h-[calc(100svh-6.5rem)] w-full max-w-6xl space-y-2 overflow-hidden">
    <div class="flex flex-col items-start justify-between gap-2 md:flex-row md:items-center">
      ...
      <div class="w-full flex flex-col gap-4 sm:w-auto sm:flex-row">
        ...
        <Button class="w-full gap-2 bg-blue-600 shadow-blue-600/30 hover:bg-blue-700 sm:w-auto" :disabled="isLoading || isSaving" @click="saveVersions">
    ...
    <div class="flex items-start gap-4 rounded-xl border border-blue-200 bg-blue-50 p-2.5 text-xs text-blue-700 shadow-sm">
      <div class="h-7 w-7 flex shrink-0 items-center justify-center rounded-full bg-blue-100 text-blue-600">
        <Info class="h-4 w-4" />
      </div>
      <div>
        <h2 class="mb-0.5 font-bold text-blue-800">Mekanisme Update</h2>
    ...
    <div v-if="isLoading" class="flex min-h-40 items-center justify-center rounded-xl border bg-white">
      <Loader2 class="h-6 w-6 animate-spin text-blue-600" />
    </div>

    <div v-else class="grid grid-cols-1 gap-6 lg:grid-cols-2">
      <Card v-for="platform in (['android', 'ios'] as Platform[])" :key="platform" class="overflow-hidden rounded-xl border-slate-200 shadow-sm">
        ...
        <CardContent class="space-y-2 p-4">
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="space-y-1.5">
              <Label class="h-5 flex items-center text-xs text-slate-700 font-bold uppercase tracking-wide">Versi Terbaru</Label>
              ...
              <p v-if="errors[`${platform}.latestVersion`]" class="text-xs text-red-600">{{ errors[`${platform}.latestVersion`] }}</p>
            </div>
            <div class="space-y-1.5">
              <Label class="h-5 flex items-center gap-1 text-xs text-rose-600 font-bold uppercase tracking-wide">Batas Minimum <TriangleAlert class="h-3.5 w-3.5" /></Label>
              ...
              <p v-if="errors[`${platform}.minRequiredVersion`]" class="text-xs text-red-600">{{ errors[`${platform}.minRequiredVersion`] }}</p>
            </div>
          </div>

          <div class="space-y-1.5">
            <Label class="text-xs text-slate-700 font-bold uppercase tracking-wide">URL {{ platform === 'android' ? 'Play Store' : 'App Store' }}</Label>
            ...
            <p v-if="errors[`${platform}.storeUrl`]" class="text-xs text-red-600">{{ errors[`${platform}.storeUrl`] }}</p>
          </div>

          <div class="space-y-1.5">
            <Label class="text-xs text-slate-700 font-bold uppercase tracking-wide">Catatan Rilis (Release Notes)</Label>
            ...
            <p v-if="errors[`${platform}.releaseNotes`]" class="text-xs text-red-600">{{ errors[`${platform}.releaseNotes`] }}</p>
          </div>
        </CardContent>
      </Card>
    </div>
