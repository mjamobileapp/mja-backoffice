...
import { ArrowDownRight, ArrowUpRight, CircleDollarSign, Filter, Loader2, ShoppingCart, X } from 'lucide-vue-next'
...
<template>
  <div class="mx-auto max-w-[1440px] space-y-5">
    <div class="flex flex-col justify-between gap-4 md:flex-row md:items-end">
      <div>
        ...
        <p class="mt-1 text-sm text-slate-500">
          Laporan ringkasan performa bisnis berdasarkan mitra, cabang, dan periode.
        </p>
      </div>
      ...
      <div class="flex flex-wrap items-end gap-3">
        <div class="min-w-[190px] flex flex-col gap-1.5">
          ...
          <select v-model="selectedMitra" class="h-10 border border-slate-200 rounded-lg bg-white px-3 text-sm shadow-sm outline-none focus:border-blue-500">
            ...
            <option v-for="item in mitraOptions" :key="item.id" :value="item.id">
              {{ item.label }}
            </option>
          </select>
        </div>
        <div class="min-w-[190px] flex flex-col gap-1.5">
          ...
          <select v-model="selectedCabang" :disabled="selectedMitra === 'all' || !cabangOptions.length" class="h-10 border border-slate-200 rounded-lg bg-white px-3 text-sm shadow-sm outline-none disabled:cursor-not-allowed focus:border-blue-500 disabled:bg-slate-50">
            <option value="all">
              {{ selectedMitra === 'all' ? 'Pilih Mitra dahulu' : 'Semua Cabang' }}
            </option>
            <option v-for="item in cabangOptions" :key="item.id" :value="item.id">
              {{ item.label }}
            </option>
          </select>
        </div>
    ...
    <div v-if="errorMessage" class="flex items-center justify-between border border-amber-200 rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-800">
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <Card class="dashboard-card metric-card metric-card-blue">
        ...
        <CardHeader class="relative z-10 h-full flex flex-col items-start p-5 space-y-0">
          ...
          <div class="mt-1 text-2xl text-[#2453cf] font-extrabold">
            {{ formatCurrency(summary.pendapatanBersih) }}
          </div>
          <p class="mt-2 text-xs text-[#2563eb] font-medium">
            Omset dikurangi pengeluaran
          </p>
        </CardHeader>
      </Card>
      ...
      <Card class="dashboard-card metric-card metric-card-amber">
        ...
        <CardHeader class="relative z-10 h-full flex flex-col items-start p-5 space-y-0">
          ...
          <div class="mt-1 text-2xl text-[#1e293b] font-extrabold">
            {{ formatNumber(summary.jumlahOrder) }}
          </div>
          <p class="mt-2 text-xs text-[#d97706] font-medium">
            Total order pada periode terpilih
          </p>
        </CardHeader>
      </Card>
    </div>

    <div v-if="loading" class="min-h-[320px] flex flex-col items-center justify-center gap-3 border rounded-xl bg-white text-sm text-slate-500">
        ...
        <Card class="border-slate-200 shadow-sm">
          ...
          <CardContent class="pt-4">
            <LineChart
              ...
              class="h-[300px]"
            />
            <div v-else class="h-[300px] flex items-center justify-center border border-slate-200 rounded-lg border-dashed bg-slate-50 text-sm text-slate-500">
