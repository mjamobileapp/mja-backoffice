...
async function fetchData() {
  try {
    ...
    const response = await fetch(
      `${baseUrl}/uploadFileSirkulerRups/${props.item.id}?username=${email.value}&t=${Date.now()}`,
      {
        method: 'GET',
        headers: {
...
async function fetchDataAkses() {
  try {
    ...
    const fetchedData = await response.json()
    // console.log(fetchedData.namaRole)
...
async function handleDownload(item: any) {
  ...
  try {
    const response: any = await $fetch(`${baseUrl}/downloadFileSirkulerRups/${item.id}`, {
      responseType: 'blob',
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
async function deleteFile(fileId: string, fileName: string) {
  if (confirm(`Apakah Anda yakin ingin menghapus file "${fileName}"?`)) {
    try {
      const response = await $fetch(`${baseUrl}/deleteFileSirkulerRups/${fileId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
          ...
          <TableBody>
            <TableRow v-for="(item, index) in paginatedData" :key="item.id">
              ...
              <TableCell class="text-center flex gap-2">
                ...
                <Button v-if="showActionButtons" @click="deleteFile(item.id, item.judulDokumen)"
                  >Hapus</Button
                >
              </TableCell>
            </TableRow>
