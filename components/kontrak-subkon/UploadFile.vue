...
const onSubmit = handleSubmit(async values => {
  ...
  formDataFile.append('idSubkon', props.item.id)

  values.files.forEach(file => formDataFile.append('files', file))

  try {
    await $fetch(`${baseUrl}/uploadBuktiDp`, {
      method: 'POST',
      body: formDataFile,
...
<template>
  <Dialog :open="isDialogOpen" @openChange="isDialogOpen = $event">
    <DialogTrigger as-child>
      <Button size="sm" @click="!props.disabled && openDialog()">
    ...
    <DialogContent class="sm:max-w-[800px] [&>button]:hidden">
      <form class="space-y-6" @submit="onSubmit">
        <DialogHeader>
          <DialogTitle>Upload Bukti DP</DialogTitle>
        </DialogHeader>
